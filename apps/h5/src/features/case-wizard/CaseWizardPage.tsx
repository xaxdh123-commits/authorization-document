...
export function CaseWizardPage({ draftClient, requirements }: { draftClient?: DraftClient; requirements?: string[] } = {}) {
  const [index, setIndex] = useState(0);
  const autosave = createAutosave(draftClient ?? { saveDraft: async () => undefined });
  const title = steps[index];
  return <div className="wizard-shell"><main onInput={e => autosave((e.target as HTMLInputElement).value)}><ol aria-label="Application steps">{steps.map((step, i) => <li key={step} aria-current={i === index ? 'step' : undefined}>{step}</li>)}</ol>{index === 0 ? <InformationStep /> : index === 1 ? <UploadStep requirements={requirements?.length ? requirements : ['Identity']} /> : index === 2 ? <PdfPreviewStep digest="pending" /> : <SubmitStep />}<div><button type="button" onClick={() => setIndex((i) => Math.max(0, i - 1))} disabled={index === 0}>Back</button>{index < steps.length - 1 && <button type="button" onClick={() => setIndex((i) => i + 1)}>Next</button>}</div></main></div>;
}

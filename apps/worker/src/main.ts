...
export async function bootstrapPdfWorker(){
  ...
  await writeWorkerHeartbeat();const heartbeatTimer=setInterval(()=>void writeWorkerHeartbeat().catch(error=>workerLog('error','WORKER_HEARTBEAT_FAILED',{error})),10_000);heartbeatTimer.unref();const timer=setInterval(()=>void enqueue().catch(error=>workerLog('error','PDF_ENQUEUE_RECONCILE_FAILED',{error})),5000);timer.unref();const worker=startWorker({start:async()=>undefined,stop:async()=>{clearInterval(timer);clearInterval(heartbeatTimer);try{await runtime.stop();}finally{await prisma.$disconnect();}}});installGracefulShutdown(worker);return worker;
}

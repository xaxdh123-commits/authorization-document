...
- Runtime entry fix `b23b183` passed the full gate and was published to GitHub. First combined remote update command was policy-rejected before execution; deployment will continue with split explicit commands.
- Server fast-forward and targeted rebuild at `b23b183` passed. A PM2 start/check wrapper containing command substitution was policy-rejected before execution; replace it with two SSH calls and local JSON status parsing.
- PM2 health remained 503 because the worker crashed on a source-level relative import of `AuditWriter`. Added two boundary regression tests, changed the import to the contracts package entrypoint, declared `@auth/contracts` as a worker runtime dependency, and verified the target worker tests/build plus a real built ESM import.
- Published `c01fd9a`, fast-forwarded the server, applied migrations and rebuilt successfully. API stayed online with zero restarts; worker then clean-exited repeatedly because both recurring timers were unreferenced. Added a process-lifetime RED/GREEN test and kept production timers referenced; full typecheck/test/build/diff gate passed.
- Published application revision `d000f72`, then identified PM2's fork wrapper as the remaining worker restart cause: it imports the guarded ESM entry instead of directly executing it. PM2 now manages `/opt/node-v22/bin/node` with the worker entry as an argument; API and worker stayed online with zero restarts and PM2 startup is enabled.
- Released Admin/H5 under `/opt/authorization-document-releases/d000f72`, atomically switched `/opt/authorization-document-current`, installed Nginx routes for `/admin/`, `/p/`, and `/api/`, and removed only the package default site after `nginx -t`. Internal and LAN checks returned Admin 200, H5 200, health 200, root 302; database/worker/storage/Chromium checks are all `ok`.
- Deployment caveats retained: real upstream `getInfo` authentication still needs its actual URL, LAN serving is HTTP-only, and this host uses its dedicated local PostgreSQL because credentials for the earlier external PostgreSQL endpoint were not provided.
